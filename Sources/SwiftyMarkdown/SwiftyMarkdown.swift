    ...
    func attributedStringFor( tokens : [Token], in line : SwiftyLine ) -> NSAttributedString {
        ...
        for token in finalTokens {
            ...
            if let linkIdx = styles.firstIndex(of: .link), linkIdx < token.metadataStrings.count {
                ...
                if usesAttachmentForLinks, let url = URL(string: token.metadataStrings[linkIdx]) {
                    attributes[.attachment] = url as AnyObject
                } else {
            ...
            if let imgIdx = styles.firstIndex(of: .image), imgIdx < token.metadataStrings.count {
                ...
                #elseif !os(watchOS)
                if #available(macOS 11.0, *), usesSystemImageName {
                    image1Attachment.image = NSImage(systemSymbolName: token.metadataStrings[imgIdx],
